...
class ConcurrencyConfig:
    ...
    # Boundary limits
    min_limits: ConcurrencyLimits = field(default_factory=lambda: ConcurrencyLimits(
        validation=15, ml=8, batch_processing=3, database=8, total=35
    ))
    max_limits: ConcurrencyLimits = field(default_factory=lambda: ConcurrencyLimits(
        validation=300, ml=150, batch_processing=25, database=80, total=500
    ))
    
    # Thresholds
    cpu_thresholds: ConcurrencyThresholds = field(default_factory=lambda: ConcurrencyThresholds(
        low=25, medium=55, high=80, critical=92
    ))
    memory_thresholds: ConcurrencyThresholds = field(default_factory=lambda: ConcurrencyThresholds(
    ...
    emergency_throttle: EmergencyThrottleConfig = field(default_factory=EmergencyThrottleConfig)
...
def _create_concurrency_config(data: Dict[str, Any]) -> ConcurrencyConfig:
    ...
    return ConcurrencyConfig(
        ...
        enable_performance_learning=concurrency_data.get("enable_performance_learning", True),
        enable_predictive_scaling=concurrency_data.get("enable_predictive_scaling", True),
...
def validate_config(cfg: AppConfig) -> None:
    ...
    if cfg.concurrency.mode not in ["conservative", "balanced", "aggressive", "adaptive"]:
    ...
    if cfg.concurrency.min_limits.batch_processing >= cfg.concurrency.max_limits.batch_processing:
...
def load_config(path: str = "config.yaml") -> AppConfig:
    ...
    feature_cfg = FeatureFlags(
        ...
        enable_dynamic_concurrency=features_data.get("enable_dynamic_concurrency", True),
        enable_performance_analytics=features_data.get("enable_performance_analytics", True),
