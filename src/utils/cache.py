...
class InMemoryCache:
    ...
    def get_stats(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "ttl": self.ttl,
            "utilization": len(self.store) / self.max_size if self.max_size > 0 else 0.0
        }
...
class DistributedCache:
        ...
        try:
            if aioredis:
                self.client = aioredis.from_url(
                    ...
                    socket_keepalive_options={"TCP_KEEPIDLE": 300, "TCP_KEEPINTVL": 30, "TCP_KEEPCNT": 3}
                )
            else:
                self.client = None
        except Exception:
    ...
    async def set(self, key: str, value: Any) -> None:
        ...
        if await self._health_check():
            ...
            try:
                data_container.clear()
                data_container.update(value if isinstance(value, dict) else {"data": value})
...
class RvuCache:
    ...
    def __init__(
        ...
        # Enhanced local caching with LRU and memory management
    ...
    def _update_local_cache(self, code: str, data: Dict[str, Any]) -> None:
        ...
        # Add new entry
        self.local_cache[code] = (current_time, dict(cache_container))
    ...
    async def get_many(self, codes: Iterable[str]) -> Dict[str, Optional[Dict[str, Any]]]:
        ...
        try:
            ...
            if missing_codes_container:
                ...
                try:
                    ...
                    if hasattr(self.db, 'get_rvu_bulk_optimized'):
                        db_results = await self.db.get_rvu_bulk_optimized(missing_codes_container)
                    else:
                        # Fallback to batch query
                        db_results = await self._fetch_batch_from_db(missing_codes_container)
            ...
            # Update metrics
            metrics.inc("rvu_cache_hits", len(local_hits_container) + len(distributed_hits_container))
            ...
            successful_codes = [code for code, data in results_container.items() if data]
    ...
    async def _bulk_predictive_load(self, accessed_codes: List[str]) -> None:
        ...
        candidates_container = self._memory_pool.acquire("bulk_predictive_candidates", set)
        ...
        try:
            ...
            candidates_container = {c for c in candidates_container if c not in accessed_codes}
            codes_to_fetch = [c for c in candidates_container if not self._get_from_local_cache(c)]
            ...
            self._memory_pool.release("bulk_predictive_candidates", candidates_container)
    ...
    async def warm_cache(self, codes: Iterable[str]) -> None:
        ...
        batch_size = min(self.batch_size * 2, 200)  # Larger batches for warming but memory-safe
    ...
    async def _check_cache_memory(self) -> None:
        ...
        try:
            ...
            if (cache_memory > self._max_cache_memory * self._cleanup_threshold or
    ...
    def _cleanup_access_patterns(self) -> None:
        ...
        if len(self.access_patterns) > 500:
            ...
            patterns_to_keep = {
                code: patterns for code, patterns in self.access_patterns.items()
        ...
        try:
            ...
            try:
                ...
                if frequent_codes_container:
                    try:
                        limited_codes = frequent_codes_container[:20]  # Limit preload size
                        await self.get_many(limited_codes)
                        optimization_container["frequent_codes_preloaded"] = len(limited_codes)
                    except Exception as e:
    ...
    async def emergency_cleanup(self) -> Dict[str, Any]:
        ...
        cleanup_results["cache_entries_removed"] = initial_cache_size - len(self.local_cache)
...
class OptimizedRvuCache(RvuCache):
    ...
    async def get_many_optimized(self, codes: Iterable[str]) -> Dict[str, Optional[Dict[str, Any]]]:
        ...
        if (self._prefetch_enabled and 
            len(self.local_cache) < self._prefetch_threshold and
            time.time() - self._last_prefetch > self._prefetch_interval):
