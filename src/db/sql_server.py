...
class MemoryMonitor:
    ...
    def __init__(
        self, warning_threshold_mb: int = 1000, critical_threshold_mb: int = 1500
    ):
...
class SQLServerDatabase(BaseDatabase):
    ...
    def __init__(self, cfg: SQLServerConfig):
        ...
        self._prepared_on_connections: set[
            int
        ] = set()  # connection IDs with prepared statements
    ...
    async def _warm_connection_pool(self) -> None:
        ...
        for i, conn in enumerate(self.pool):
            try:
                ...
                self._connection_memory_tracking[id(conn)] = (
        ...
        print(
            f"SQL Server pool warmed: {warmed_count} successful, {failed_count} failed"
        )
        ...
        async with self._lock:
            ...
            for i, conn in enumerate(self.pool):
                try:
                    ...
                    print(
                        f"Warning: Failed to prepare statement {statement_name} on connection {i}: {e}"
                    )
    ...
    async def execute_many_with_memory_management(
        ...
        *,
        ...
        safe_batch_size = await self._calculate_safe_batch_size(
            len(params_list), batch_size
        )
    ...
    async def _process_chunk_with_memory_monitoring(
    ...
    async def _calculate_safe_batch_size(
        self, total_records: int, requested_batch_size: int
    ) -> int:
        """Calculate safe batch size based on memory constraints."""
        try:
            process_memory = self._get_process_memory()
            available_memory = (
                psutil.virtual_memory().available / 1024 / 1024 if psutil else 1000
            )  # MB
    ...
    async def fetch_optimized_with_memory_management(
        ...
        *params: Any,
        ...
        try:
            ...
            if memory_growth > 50:  # More than 50MB growth
                print(
                    f"Warning: High memory growth in SQL fetch: {memory_growth:.1f}MB"
                )
                await self._routine_memory_cleanup()
    ...
    async def _cache_with_memory_management(self, cache_key: str, data: Any) -> None:
        ...
        if self._current_cache_memory + estimated_size <= self._cache_max_memory:
            ...
            await self._cleanup_cache_by_memory()
            if (
                estimated_size <= self._cache_max_memory // 2
    ...
    async def bulk_insert_tvp_optimized(
        ...
        *,
        ...
        for i in range(0, len(rows_list), chunk_size):
            ...
            tvp_insert = (
                f"INSERT INTO {table} ({', '.join(columns_list)}) SELECT * FROM ?"
            )
            ...
            try:
                ...
                try:
                    ...
                    if hasattr(cursor, "setinputsizes") and hasattr(
                        pyodbc, "SQL_STRUCTURED"
                    ):
    ...
    def get_optimization_stats(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "memory_management": {
                "process_memory_mb": self._get_process_memory(),
                "memory_checks_performed": self._operation_count
                // self._memory_check_frequency,
                "memory_pool_stats": self._memory_pool.get_stats(),
            },
